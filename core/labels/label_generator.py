...
def generate_labels(
    ...
    """
    Generate Buy/Hold/Sell labels based on future returns (3-CLASS)
    ...
    Returns:
        ...
        - labels_series: Buy(1), Hold(0), or Sell(-1)
        - metadata_dict: Label distribution and statistics
    
    Logic:
        - BUY (1): Future return >= threshold (expect significant gain)
        - HOLD (0): Future return between -threshold and +threshold (no clear signal)
        - SELL (-1): Future return <= -threshold (expect significant loss)
    """
    ...
    # Generate labels - 3-CLASS (BUY, HOLD, SELL)
    # BUY if future return >= threshold
    # SELL if future return <= -threshold
    # HOLD if between -threshold and +threshold
    labels = pd.Series(0, index=df.index)  # Default: HOLD (0)
    labels[forward_return >= adjusted_threshold] = 1   # BUY (significant up move)
    labels[forward_return <= -adjusted_threshold] = -1   # SELL (significant down move)
...
def print_label_distribution(distribution: Dict):
    ...
    print(f"\n   📊 Label Distribution (3-Class: BUY, HOLD, SELL):")
    print(f"      Total: {distribution['total']}")
    print(f"      BUY:   {distribution['buy']:4d} ({distribution['buy_pct']:5.1f}%)")
    print(f"      HOLD:  {distribution['hold']:4d} ({distribution['hold_pct']:5.1f}%)")
    ...
    max_pct = max(distribution['buy_pct'], distribution['sell_pct'], distribution['hold_pct'])
    if max_pct > 70:
        print(f"\n      ⚠️  Severe class imbalance! ({max_pct:.1f}% dominated)")
        print(f"      Consider adjusting threshold or forward_days")
