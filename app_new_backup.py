...
def create_trade_markers_chart(df: pd.DataFrame, trades: list, ticker: str) -> go.Figure:
...
def main():
    ...
    if not st.session_state.analysis_complete:
        ...
        with st.expander("📊 Stock Data & Technical Indicators", expanded=True):
            ...
            st.divider()
            st.markdown("**📉 Price Chart (Recent 60 Days):**")
            recent_data = system.feature_data.tail(60)
        ...
        with tab2:
            ...
            fig_trades = create_trade_markers_chart(system.feature_data, trades, system.ticker)
            st.plotly_chart(fig_trades, use_container_width=True)
            
            st.caption(f"🟢 Green = Buy | 🔴 Red = Sell | 🟠 Orange = Stop Loss")
